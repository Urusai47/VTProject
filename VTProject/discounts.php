...
if ($_POST['delete'] and $_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $discountname = mysqli_real_escape_string($db, $_POST["discountname"]);

    if (ctype_space($percentage) || ctype_space($discountname) || $percentage == '' || $discountname == '') {
        $error = 'At least one of the field is empty';
    }  
    else {
        
        $sql = "CALL insert_discount('" . $percentage . "' , '" . $discountname . "' , '" . $_SESSION["UserName"] . "')";
        $result = mysqli_query($db, $sql);       
    }
} 
else {}
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-5">
                        <!-- Logo -->
                        <div class="logo">
                            <h1><a href="discounts.php">Discounts</a></h1>
                        </div>
                    </div>             
                </div>
            </div>
                        ...
                        <div class="content-box-large">
                            <div class ="row ">
                                <form action="" method="post">  
                                    ...
                                    <div class="action">
                                        <input class="btn btn-primary signup" type = "submit" value = " Add Discount "/><br />
                                    </div>    
                                </form>
                            </div>
                        </div>
                            ...
                            <div class="panel-body">
                                <table class="table table-striped table-bordered" id="example" cellspacing="0" cellpadding="0" border="0">
                                    ...
                                    <tbody>
                                        ...
                                        $query = "CALL get_discounts()";
                                        $result = mysqli_query($db, $query);
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo '<tr>';
                                            echo "<td>" . $row["Percentage"] . "</td> <td>" . $row["Name"] . "</td> ";
     
                                            echo '<form action="" method="post">';
                                            echo "</td> <td> <input type ='submit' class='button' name='delete' value='Delete' /> "
                                            . "<input type ='hidden' name='delete' value='".$row["DiscountID"]."' /> </td>";
                                            echo '</form>';
